...
def main(config):
    ...
    #Find index limits
    pw_gw = regressors['pw'] / regressors['gw']
    regressors['pw_gw'] = pw_gw
    mean = np.mean(pw_gw)
    std = np.std(pw_gw)
    low_limit = mean - std*1.28
    high_limit = mean + std*1.28
    index_limits = {'low_pw':low_limit,'mean_pw':mean,'high_pw':high_limit}
    ...
    df.to_csv(config["work_dir"]+'/remote_drivers_polar_warming_global_warming.csv')
    df = pd.DataFrame(index_limits,index=[0])
    df.to_csv(config["work_dir"]+'/remote_drivers_polar_warming_global_warming_bounds.csv')
